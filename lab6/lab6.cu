...
__global__ void mul_gpu(int* a, int* b, int* c) {
    ...
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid <= N / 256)
        c[tid] = abs(a[tid]) * abs(b[tid]) * cosf((a[tid] * b[tid]) / (abs(a[tid])) * abs(b[tid]));
}
int main()
{
    //srand(time(0));
    ...
    for (int i = 0; i < FULL_DATA_SIZE; i += N * 2) {
        cudaMemcpyAsync(dev_a2, host_a + i, N * sizeof(int), cudaMemcpyHostToDevice, stream2);
        cudaMemcpyAsync(dev_b2, host_b + i, N * sizeof(int), cudaMemcpyHostToDevice, stream2);
        mul_gpu << <N / 256, 256, 0, stream2 >> > (dev_a2, dev_b2, dev_c2);
        cudaMemcpyAsync(host_c + i, dev_c2, N * sizeof(int), cudaMemcpyDeviceToHost, stream2);

        cudaMemcpyAsync(dev_a3, host_a + i + N, N * sizeof(int), cudaMemcpyHostToDevice, stream3);
        cudaMemcpyAsync(dev_b3, host_b + i + N, N * sizeof(int), cudaMemcpyHostToDevice, stream3);
        mul_gpu << <N / 256, 256, 0, stream3 >> > (dev_a3, dev_b3, dev_c3);



        cudaMemcpyAsync(host_c + i + N, dev_c3, N * sizeof(int), cudaMemcpyDeviceToHost, stream3);
    }   
